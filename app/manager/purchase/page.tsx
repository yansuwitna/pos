...
export default function PurchasePage() {
  ...
  const startScanner = () => {
    setScanning(true);
    setTimeout(() => {
      const scanner = new Html5QrcodeScanner("sku-reader-purchase", { 
        fps: 15,
  ...
  const addToCart = async (product: Product) => {
    ...
    if (existing) {
      ...
      try {
        ...
        const data = await res.json();
        if (data.success) cost = data.product.cost;
      } catch (e) {}
      
      setCart([...cart, { product, quantity: 1, unitCost: cost }]);
    }
  };
  ...
  const submitPurchase = async () => {
    ...
    try {
      ...
      if (data.success) {
        Swal.fire('Berhasil!', "Restock berhasil dicatat. Stok barang bertambah.", 'success');
        setCart([]);
        setSelectedSupplier('');
    ...
    <div className="grid-2">
      <div className="card" style={{ gridColumn: showForm ? 'unset' : '1 / -1' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          {!showForm && (
            <button className="btn btn-success" onClick={() => setShowForm(true)}>➕ Tambah Pembelian</button>
          )}
        </div>
        ...
        <div className="table-container">
          ...
          {history.length === 0 && <p style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Belum ada data pembelian.</p>}
        </div>
      </div>
