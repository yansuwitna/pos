    ...
    <div>
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h2 className="card-title" style={{ marginBottom: 0 }}>📋 Daftar Pelanggan ({filteredCustomers.length})</h2>
          
          <div style={{ display: 'flex', gap: '1rem', flex: '1 1 300px', justifyContent: 'flex-end' }}>
            <input 
              type="text" 
              placeholder="🔍 Cari nama atau nomor HP..." 
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              style={{ padding: '0.5rem', borderRadius: '8px', border: '1px solid var(--border)', maxWidth: '300px', width: '100%' }}
            />
            {!showForm && !roleLoading && userRole !== 'ADMIN' && (
              <button className="btn btn-success" onClick={() => setShowForm(true)} style={{ whiteSpace: 'nowrap' }}>➕ Tambah</button>
            )}
          </div>
        </div>
        <div className="table-container">
          ...
          {!tableLoading && filteredCustomers.length === 0 && <p style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Belum ada data pelanggan yang sesuai.</p>}
        </div>
      </div>
